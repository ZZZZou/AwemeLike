...
lowp vec3 rgb2hsv(lowp vec3 c) { lowp vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
...
lowp vec3 ContrastSaturationBrightness(lowp vec3 color, lowp float brt, lowp float sat, lowp float con) {
    ...
    lowp vec3 brtColor = color * brt; lowp vec3 intensity = vec3(dot(brtColor, LumCoeff));
...
void main()
{
    lowp vec4 inputColor = texture2D(textureSampler, textureCoordinate);
